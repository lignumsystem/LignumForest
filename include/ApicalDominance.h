...
namespace LignumForest{
  ...
  ///\brief Reduce apicality in the top part of the crown based on Gravelius order
  ///
  ...
  ///\brief Reduce apicality based on relative light position in the top part of the crown
  ///
  ///Reduce apicality based on tree age, segment relative height in tree crown and relative light position.
  ...
  public:
    ...
    ///\param top Top part of the tree crown
    ///\param apical Apicality reduction as a function of Gravelius order
    ReduceApicalityWithFip(double lambda, double age, double top, const ParametricCurve& apical)
      :tree_apical_age(age),crown_top(top),fapicality(apical),tree_height(0),tree_height_calculated(false){}
    ///\brief Segment length with decreased growth potential for reduced apicality
    ///
    ///Segment length is based on LignumForest::SetScotsPineSegmentLength Basic model
  ...
  private:
    ...
    ///\sa Lignum::FindHighestPoint
    mutable double tree_height;
    ///Boolean value if the tree height has been calculated once \sa ReduceApicalityWithFip::operator()
    mutable bool tree_height_calculated;
  };
  ...
  TreeCompartment<ScotsPineSegment,ScotsPineBud>* ReduceApicalityWithFip::operator()(TreeCompartment<ScotsPineSegment,ScotsPineBud>* tc)const
  {
	...
	if ((GetValue(GetTree(*ts),LGAage) >= tree_apical_age) && ((z / tree_height) >= crown_top)){
	  ///\par Reduce apicality
	  ///If the tree age and the segment height  in the tree crown triggers the tree crown apicality model
	  ///reduce the segment elongation defined by relative light position
	  ///\internal
