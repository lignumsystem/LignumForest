# HDF5 files
Each simulation run creates three HDF5 files — one for the primary results, a second 
for simulated trees and the third for the voxel space data.

File names follow the user-defined primary HDF5 result file, with *TreesXML* or *VoxelSpaces* 
appended for tree and voxel space data.
...
- <I>/ForestDeadTreeData</I>: HDF5 3D table for data from dead trees. 
  Dead trees appear only once in the year they are removed from the simulation.
- <I>/StandData</I>: HDF5 2D table for aggregate stand data.
- <I>/CenterStandData</I>: HDF5 2D table for center stand data.

### Functions and parameters
The parameter and function text files are stored in their original format as string datasets.
...
### Firmament
Firmament data (possibly multiple instances) are stored in their original format.
- <I>/Firmament/Firmament\*.txt</I>: Firmament configuration saved as a string. 

### Voxel space
The obsolete datasets track voxel space size dynamics and will be removed, as this data is now also
in the primary voxelspace HDF5 file alongside detailed tree structure, foliage, and solar radiation data. 
...
### Supplementary data
Obsolete supplementary data, which replicate function and parameter file datasets, will be removed from
the HDF5 file structure.

- <I>/AllFunctions</I>: Group for functions. Functions are saved as HDF5 2D tables.
- <I>/TreeFunctions</I>: Group for Lignum::Tree tree functions. Function files are save as HDF5 2D tables.
- <I>/AllParameters</I>: Group for parameters. Parameter values are saved as HDF5 2D tables.
- <I>/Parameters/TreeParameters</I>: Lignum::Tree parameters using Lignum::LGMPD names as HDF5 1D array.

## Tree architecture
Trees are stored as XML strings and grouped by simulation year. Dataset names are based on unique tree ID tags,
and collection occurs at user-defined intervals, for example:

- <I>/TreeXML</I>: Main group for the trees.
- <I>/TreeXML/10</I>: Group for trees collected in year 10.
- <I>/TreeXML/20/Tree_967</I>: Tree with ID tag 967, collected in year 20.

## Voxel space data
*VoxelSpace* datasets are collected at regular intervals as 4D matrices, where the fourth dimension represents 
the tree structure, foliage and solar radiation data recorded from each voxel. 
